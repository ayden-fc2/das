...
export default function AnalysisPage() {
    ...
    useEffect(() => {
        ...
        if (!basicInfo?.jsonPath) {
            setTimeout(()=> {
                err('Please select a valid project to analysis')
                setLoading(false);
                setNoProject(true)
            }, 0)
        } else {
    ...
    return (
        <Box className={`relative w-full h-full overflow-hidden rounded-md shadow-md`}>
            <CanvasComponent
                ref={canvasComRef}
            ...
            {noProject && (
                <Box className={`absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-center text-gray-500  w-full h-full z-50 flex justify-center items-center bg-white`}>
                    <p className={`text-lg`}>Please select a valid project from <strong>Demo Page</strong> or <strong>Projects Page</strong></p>
                </Box>
            )}
        </Box>
    );
