...
const ResizableDialog = React.memo( ({isOpen, onClose, children, loading} : ResizableProps) => {
    ...
    return (
        isOpen && (
            loading? (
                <Box className="absolute top-0 left-0 w-full h-full bg-gray-900 bg-opacity-50 z-50 flex justify-center items-center">
            ...
            (
                <PanelGroup className={`absolute h-full right-0 left-0 top-0 z-20 bg-transparent pointer-events-none`} direction="vertical">
                    <Panel defaultSize={200} className={`bg-transparent pointer-events-none`}/>
                    <PanelResizeHandle />
                    <Panel defaultSize={120} className="relative pointer-events-auto rounded bg-white/80 backdrop-blur-sm px-4 pb-2 pt-0 border-t-2">
